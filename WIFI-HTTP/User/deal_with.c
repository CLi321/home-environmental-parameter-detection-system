...
//    NVICPriority_Structure.Usart3 = 2;
    ...
    IWDG_Init(6,2500);    //16S一次
		
		RELAY(RELAY_ON);			//继电器控制esp8266硬件复位
		delay_ms(100);
		RELAY(RELAY_OFF);		
...
void POST_Data(void)
{
		...
		numToString((u16)STM32.sensor_temp);
		printf("temp: %s, ", strValue);
		esp8266_str_data("temp", strValue);	
	
		numToString((u16)STM32.sensor_hum);
		printf("hum: %s, ", strValue);
		esp8266_str_data("hum", strValue);

		numToString((u16)STM32.sensor_light);
		printf("light: %s, ", strValue);
		esp8266_str_data("light", strValue);
	
		numToString((u16)STM32.sensor_MQ2);
		printf("MQ2: %s, ", strValue);
		esp8266_str_data("MQ2", strValue);

		numToString((u16)STM32.sensor_Voltage);
		printf("Fire: %s\r\n\r\n", strValue);
		esp8266_str_data("Fire", strValue);	
	
	
//		char str[30] = {0};		// 假设最多需要20个字符的空间
//		sprintf(str, "%.d", STM32.sensor_light);
//		printf("light: %s, ", str);
//		esp8266_str_data("light", str);
...
//		relayctr = esp8266_get_data("relay");
//		if(relayctr == 1) 
//		{
//				RELAY(RELAY_ON);
//		}
